# Ruby notes:
# this is a totally naive port of addresses.py from the ebdata package
# located here: https://code.google.com/p/ebcode/
...
module EbdataAddresses
    ...
    STREET_NAME_CAPTURE_RE = Regexp.new( STREET_NAME_CAPTURE, Regexp::EXTENDED)
    STREET_NAME_NOCAPTURE_RE = Regexp.new( STREET_NAME_NOCAPTURE, Regexp::EXTENDED)

    addresses_re_string = <<-eos
        ...
        (?:
            ...
            (?:
                %{STREET_NAME_CAPTURE} (,?[ ]+ between [ ]+) %{STREET_NAME_CAPTURE} (,?[ ]+ and [ ]+) %{STREET_NAME_CAPTURE}
                |
                %{STREET_NAME_CAPTURE} (,?[ ]+ from [ ]+) %{STREET_NAME_CAPTURE} (,?[ ]+ to [ ]+) %{STREET_NAME_CAPTURE}
            )
    ...
    eos
    ADDRESSES_RE = Regexp.new( (addresses_re_string % {:STREET_NAME_CAPTURE => STREET_NAME_CAPTURE, :STREET_NAME_NOCAPTURE => STREET_NAME_NOCAPTURE}), Regexp::EXTENDED)

    # Returns a list of all addresses found in the given string, as tuples in the
    # format (address, city).
    def self.parse_addresses(text)
        ...
        puts text.scan(ADDRESSES_RE).inspect
        text.scan(ADDRESSES_RE).map{|bits| [bits[0...-1].join(''), bits[-1]] }
    end
    ...
    def self.tag_addresses(text, pre='<addr>', post='</addr>')
        def _re_handle_address(m)
            bits = m.groups()
            return pre + ''.join(filter(None, bits[0...-1])) + (bits[-1] and (', %s' % bits[-1]) or '') + post
        end
